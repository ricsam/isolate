...
export async function executeLocatorAction(
...
export async function executeExpectAssertion(
  locator: PlaywrightLocator,
  matcher: string,
  expected: unknown,
  negated: boolean,
...
export async function executePageExpectAssertion(
  page: Page,
  matcher: string,
  expected: unknown,
  negated: boolean,
    ...
    try {
      ...
      switch (op.type) {
        ...
        case "waitForResponseStart": {
          ...
          const responsePromise = targetPage.waitForResponse(urlOrPredicate, { timeout: effectiveTimeout });
          const listenerId = `resp_${registry.nextListenerId++}`;
          registry.pendingResponses.set(listenerId, responsePromise);
          return { ok: true, value: { listenerId } };
        }
        case "waitForResponseFinish": {
          const [listenerId] = op.args as [string];
          const responsePromise = registry.pendingResponses.get(listenerId);
          if (!responsePromise) {
            return { ok: false, error: { name: "Error", message: `No pending response listener: ${listenerId}` } };
          }
          const response = await responsePromise;
          registry.pendingResponses.delete(listenerId);

          // Eagerly resolve all response data
          const text = await response.text();
          let json: unknown = null;
