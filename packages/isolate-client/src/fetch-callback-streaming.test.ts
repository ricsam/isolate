...
describe("Fetch Callback Streaming", () => {
  ...
  it("should demonstrate that response.body.tee() fails on external fetch response", async () => {
    const runtime = await connection.createRuntime({
      fetch: async (request) => {
        return fetch(request);
      },
    ...
    try {
        ...
        serve({
          async fetch(request) {
            ...
            try {
              ...
              // Try to tee the stream - this should work with a real ReadableStream
              // but fails because the body is just a buffer
              const bodyType = typeof externalResponse.body;
              const hasTee = typeof externalResponse.body?.tee === "function";
              const bodyConstructor = externalResponse.body?.constructor?.name;
              ...
              if (hasTee) {
                const [s1, s2] = externalResponse.body.tee();
                globalThis.testResult.teeWorked = true;
              }

              return new Response(JSON.stringify(globalThis.testResult), {
                headers: { "Content-Type": "application/json" }
              });
            } catch (error) {
              globalThis.testError = error.message;
              return new Response(JSON.stringify({
                error: error.message,
                result: globalThis.testResult
              }), {
                status: 500,
                headers: { "Content-Type": "application/json" }
              });
      ...
      console.log("Response body analysis:", data);

      // External fetch response body should be a proper ReadableStream with tee() method
      // Currently FAILS because the body is a HostBackedReadableStream without tee()
      assert.strictEqual(
        data.hasTee,
        true,
        `External fetch response.body should have tee() method, but hasTee=${data.hasTee}. ` +
          `Body constructor is '${data.bodyConstructor}' instead of a proper ReadableStream.`
      );

      if (data.error) {
        assert.fail(
          `tee() should work on external fetch response body, but got error: ${data.error}`
        );
      }
    } finally {
      await runtime.dispose();
