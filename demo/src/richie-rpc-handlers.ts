/**
 * richie-rpc handler code that runs inside the sandboxed Isolate environment.
 *
...
function generateAIResponse(prompt: string): string[] {
  const responses: Record<string, string> = {
    default: 'I am a simulated AI response running inside Isolate. This demonstrates streaming capabilities where each word is sent progressively to the client with natural-feeling delays between tokens. Pretty cool, right?',
    hello: 'Hello! Welcome to the Isolate AI demo. I can stream responses word by word, simulating how large language models generate text incrementally. Try asking me to explain something!',
...
serve({
  async fetch(request, server) {
    ...
    if (upgradeData) {
      // Data stays within Isolate, connectionId is generated by server.upgrade()
